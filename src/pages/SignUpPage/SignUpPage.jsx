            ...
            <div className="signUpPageContainer">
                ...
                <form onSubmit={handleSubmit}>
                    <div className="inputContainer">
                        ...
                        {!usernameValid && <p className="error">Username must be alphanumeric</p>}
                        <input className="inputNumber" type='text' placeholder='Contact Number' value={contactNumber} onChange={handleContactNumberChange} />
                        {!contactNumberValid && <p className="error">Please enter a contact number</p>}
                        <input className="inputEmail" type='text' placeholder='Email' value={email} onChange={handleEmailChange} />
                        {!emailValid && <p className="error">Please enter a valid email</p>}
                        <input className="inputPassword" type='password' placeholder='Password' value={password} onChange={handlePasswordChange} />
                        {!passwordValid && <p className="error">Password must be at least 8 characters long and contain at least one digit, one lowercase letter, and one uppercase letter</p>}
                    </div>
                        <p className="linkToLogIn">Already have an account? <Link to="/LoginPage">Log In</Link></p>
                    <div className="buttonContainer">
                        <button className="submitBtn" type="submit" disabled={!usernameValid || !contactNumberValid || !emailValid || !passwordValid}>Submit</button>
                    </div>
                </form>
            </div>
        </div>
